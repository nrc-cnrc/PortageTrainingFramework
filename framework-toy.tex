...
each experiment is done in a separate copy of the framework.
...
user documentation (found in
\texttt{PORTAGEshared/doc/user-manual.html} on the CD).
For detailed information on a particular program, run it
...
(section~\ref{COW}) and rescoring weights (section~\ref{RAT}), and for testing
(section~\ref{Testing}).\footnote{In this example, we use separate corpora for
  tuning decoder and rescoring weights, but this is not necessary.}  Typically,
the latter three corpora are around 1000 segments each.  If the corpus is
chronological, then it is a good idea to choose these three corpora from the
most recent material, which is likely to resemble future text more closely.

Since proper splitting of a corpus needs to take into account its structure
and nature, these steps are not handled by the experimental
framework. For the toy experiment, we provide small data sets that can be found
here: \texttt{PORTAGEshared/test-suite/unit-testing/framework-toy}.  These sets
are very small to reduce running time, so the resulting translations are of
poor quality.
...
\item select a language modeling option (see below for more info about this
  choice): 
\begin{itemize}
\item to use SRILM, no change is required
\item to use IRSLTM, comment out the \texttt{LM\_TOOLKIT\_SRI} variable and set the
\texttt{IRSTLM} variable to the location where IRSTLM is installed
\item to use neither, comment out both \texttt{LM\_TOOLKIT\_SRI} and \texttt{IRSTLM}
\end{itemize}
\end{itemize}
...
We recommend you use SRILM www.speech.cs.cmu.edu/ SLM\_info.html) as your
language modeling toolkit, if your licensing requirements permit it; if not,
you can use IRSTLM instead (irstlm.sourceforge.net).\footnote{Another options
  is the CMU SLM (www.speech.cs.cmu.edu/ SLM\_info.html).} If you have neither
toolkit at the moment, we provide trained language models that you can drop
into the framework to finish this toy experiment while skipping language model
training. To use IRSTLM, in addition to the edits described above, you need to
type \texttt{make help} and cut and paste the two export commands, unless
you've already done the equivalent steps while installing IRSTLM:
\begin{verbatim}
   > make help | head -3
...
variables (the default is conservatively set to 1).  If you are running on a
...
instead of being submitted to the queue, while \texttt{RP\_PSUB\_OPTS=...} is
used to specify additional options to \texttt{psub}, which is used in \PS to
encapsulate the invocation of \texttt{qsub}.  If your cluster has specific
usage rules or if you require additional parameters to \texttt{qsub}, you might
want to customize \texttt{psub} itself or add options as required in this
framework.
...
this is only for the input to \texttt{canoe}).
...
accepts models in the widely-used ``DARPA'' format which is produced by
...
\texttt{toy.experiment} directory (\texttt{cd ..} from the corpora directory if
...
converts it into the Portage binary language model format for quicker loading.
...
language model format for quicker loading.
...
from IBM2 word alignments, and one from HMM word alignments.  You can do all
this by typing \texttt{make tm} in your \texttt{toy.experiment} directory, but
we'll break it down into several steps here.
...
smoothers to calculate the probabilities.\footnote{In general, we often obtain
  better translation quality when using phrase probabilities estimated from relative
frequencies (``RFSmoother'') along with relative frequencies with Kneser-Ney smoothing
(``KNSmoother'') and Zens-Ney lexical smoothing (``ZNSmoother''). Other
smoothing options are also available.}
Although
output together in the same phrase table, they are separate probability models,
whose relative weights will be tuned during decoder weight optimization (see
section~\ref{COW}).
\begin{verbatim}
   > make ibm2_cpt
...
\end{verbatim}
The phrase table \texttt{cpt.ibm2.train.fr-en.gz}, jointly with its HMM counterpart
...
\end{verbatim}
\end{footnotesize}
You'll notice there are in fact six numbers per line.\footnote{With more
  digits than shown here.}  That's because we used
...
phrase table are not such good translations.
...
recipes to produce two of them: one similar to the baseline described in Liang
et al (ACL 2006), invoked via the \texttt{hmm2\_*} targets; and
one with lexically conditioned jump parameters, following He (WMT-2006),
invoked via the \texttt{hmm1\_*} targets. Many other variants are available, as
documented by \texttt{train\_ibm -h}. To select one or more of them, you'll
need to modify \texttt{models/tm/Makefile}.  It's probably easier to modify the
commands associated with the \texttt{hmm1\_model} and \texttt{hmm2\_model}
targets than to create new targets.

To install your choice as the default, change the \texttt{PT\_TYPES} variable
in \texttt{toy.experiment/Makefile.params} to, for instance, \texttt{ibm2\_cpt
hmm2\_cpt}: this variable enumerates which models you want to generate when you
type \texttt{make all}. ({\bf Warning}: if you plan to ``automate'' any of the steps
in this example using \texttt{make all}, you must set \texttt{PT\_TYPES} to
\texttt{ibm2\_cpt hmm2\_cpt} in order to have this work properly.)
All models you generate will be used in subsequent steps,
so only generate the ones you intend to use.  (You should work in different
copies of the framework if you want to experiment with different variants.)
...
\texttt{toy.experiment} directory, but we'll break it down into several steps
here.
...
\texttt{canoe.ini} file produced is error-free---it checks that all parameters
...
files to a temporary work directory called \texttt{foos}. To prevent these
(large) files from being automatically cleaned up on completion, remove the
\texttt{rm} command executed after \texttt{cow.sh} in \texttt{Makefile}.  A lot
of information is also logged to \texttt{log.canoe.ini.cow}; this can be
summarized using the program \texttt{cowpie.sh}.
...
which we need to override manually here to demonstrate translation by decoding
only:
\begin{verbatim}
   > make translate DO_RESCORING=
   ln -sf ../models models
