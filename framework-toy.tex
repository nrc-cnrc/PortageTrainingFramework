...
licensing requirements permit it).
If you have neither, leave both variables undefined; we provide trained
language models that you can drop into the framework to finish this toy
experiment while skipping language model training.
...
experiments without an LM toolkit.
...
instructed you to type \texttt{make hmm2\_cpt} above.  To have \texttt{make
all} make the same choice, change the \texttt{PT\_TYPES} variable in
\texttt{Makefile} to \texttt{ibm2\_cpt hmm2\_cpt}: this variable enumerates
which models you want to generate when you type \texttt{make all}.  All models
you generate will be used in subsequent steps, so only generated the ones you
intend to use.  (You should work in different copies of the framework if you
want to experiment with different variants.)
