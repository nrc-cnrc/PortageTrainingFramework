...
need access to the models.  The command replaces the template parameters by the
appropriate values:
\begin{itemize}
...
\item \texttt{<FORWARD\_WEIGHTS>}: the right number of forward translation
model weights.  This special token is required to enable the use of forward
weights by \texttt{canoe} and their tuning by \texttt{cow.sh}.
Unlike other weigths, which are used by default as soon as
the corresponding model is present, forward weights are disabled by default.
Therefore, the appropriate number of 1's must be inserted into
\texttt{canoe.ini} for its \texttt{[weight-f]} parameter.  In this framework,
if you don't want to use forward weights, comment out the \texttt{[weight-f]}
line in \texttt{canoe.ini.template}.
\end{itemize}
