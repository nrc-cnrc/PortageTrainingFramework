...
\begin{verbatim}
   ...
   please run the following in order for this framework
   to run properly:
...
\begin{verbatim}
   ...
   ngram-count -interpolate -kndiscount -order 5 \
      -text ../../corpora/train_en.lc.gz \
      -lm train_en-kn-5g.lm.gz \
...
\begin{verbatim}
   ...
   zcat ../../corpora/train_en.lc.gz \
      | estimate-ngram -order 5 -smoothing ModKN \
        -text /dev/stdin -write-lm train_en-kn-5g.lm.gz
...
\begin{verbatim}
   ...
   build-lm.sh -p -t stat.$$ -n 5 -k 4 -s kneser-ney \
      -i "gunzip -c train_en.marked.gz" \
      -o train_en-kn-5g.ilm.gz \
...
model, \texttt{train\_en-kn-5g.lm.gz} in
...
If you have not installed any LM toolkit, we provide
...
\begin{verbatim}
   > make ibm2_cpt
   joint2cond_phrase_tables -sort -prune1 100 -v -i -z \
...
\begin{verbatim}
   ...
   cat canoe.ini.template \
      | sed -e 's/<SL>/fr/g' \
            -e 's/<TL>/en/g' \
            -e 's#<CPTS>#models/tm/cpt.hmm3-rf-zn.train.[...]' \
   ...
   cat tmp.canoe.ini \
      ...
      | configtool -p "args:-dist-phrase-swap -dist-limit-ext \
           -lex-dist-model-file models/ldm/ldm.hmm3+ibm2.[...]" \
           > canoe.ini
   rm tmp.canoe.ini
...
\begin{verbatim}
   ...
   cat rescore-model.template \
      | sed -e "s#IBM\\(.\\)FWD#models/tm/ibm\\1.train.[...] \
            -e [...] \
      >> rescore-model.ini
...
\begin{verbatim}
   ...
   IBM1TgtGivenSrc:models/tm/ibm1.train.en_given_fr.gz -0.0004155421
   ...
   HMMVitTgtGivenSrc:models/tm/hmm3.train.en_given_fr.gz -0.00428659
   HMMVitSrcGivenTgt:models/tm/hmm3.train.fr_given_en.gz 0.001487048
   IBM1WTransTgtGivenSrc:models/tm/ibm1.train.en_given_fr.gz 0.01120
   IBM1WTransSrcGivenTgt:models/tm/ibm1.train.fr_given_en.gz -0.0190
   IBM1DeletionTgtGivenSrc:models/tm/ibm1.train.en_given_fr.gz -1
   IBM1DeletionSrcGivenTgt:models/tm/ibm1.train.fr_given_en.gz 0.433
...
done by \texttt{ce\_translate.pl}.  Truecasing can optionally be performed by
