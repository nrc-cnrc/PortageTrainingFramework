...
use separate corpora for tuning decoder and rescoring weights, but this is not
...
rescoring tuning set.} Typically, the tuning and testing corpora are 1000 to
2000 segments each.  If the corpus is chronological, then it is a good idea to
choose these corpora from the most recent material, which is likely to resemble
future text more closely.
...
process for SRILM and MITLM.\footnote{In this toy example, we manually added a
few sentences to the target language part of the parallel training corpus to
illustrate using more text to train the language model than the translation
model.  If you have access to some additional monolingual text, adding it to
your main LM training corpus (\code{TRAIN\us{}LM})
is the simplest option.  If you have access to large amounts of
monolingual text in your target language, you can use them to train additional
language models, or mixture language models.  TODO REWORK THIS FOOTNOTE ONCE
MIXLM STUFF IS WRITTEN.  The framework does not support training multiple
language models TODO CHECK IF THIS IS STILL TRUE, but you can train them
externally and add them to the \code{canoe.ini.template} file (see
\S\ref{COW}).} Commands are assumed to be issued from the \code{toy.experiment}
...
\begin{alltt}
   ...
   ngram-count -interpolate -kndiscount -order 5 \bs
      -text ../../corpora/lm-train_en.lc.gz -lm lm-train_en-kn-5g.lm.gz \bs
      >& log.lm-train_en-kn-5g.lm
   arpalm2binlm lm-train_en-kn-5g.lm.gz lm-train_en-kn-5g.binlm.gz \bs
      >& log.lm-train_en-kn-5g.binlm
\end{alltt}
\end{small}
...
\begin{alltt}
   ...
   zcat ../../corpora/lm-train_en.lc.gz \bs
      ...
      | estimate-ngram -order 5 -smoothing ModKN -text /dev/stdin \bs
        -write-lm lm-train_en-kn-5g.lm.gz >& log.lm-train_en-kn-5g.lm
   arpalm2binlm lm-train_en-kn-5g.lm.gz lm-train_en-kn-5g.binlm.gz \bs
      >& log.lm-train_en-kn-5g.binlm
\end{alltt}
\end{small}
...
\begin{alltt}
   ...
   zcat -f ../../corpora/lm-train_en.tc.gz \bs
        ...
        | utokenize.pl -pretok -paraline -ss -p -lang=en 2> log.lm-train_en.tokss \bs
...
\begin{alltt}
   normc1 -ignore 1 -extended -notitle -loc en_CA.UTF-8 lm-train_en.nc1.binlm.gz \bs
        lm-train_en.tokss.gz 2> log.lm-train_en.nc1 \bs
...
\begin{alltt}
   zcat -f lm-train_en.nc1.gz |  utf8_casemap -c l \bs
        | compile_truecase_map lm-train_en.nc1.gz - 2> log.lm-train_en.map > lm-train_en.map
\end{alltt}
\end{small}
...
\begin{alltt}
   zcat -f ../../corpora/lm-train_fr.al.gz \bs
        | utokenize.pl -pretok -paraline -ss -lang=fr 2> log.lm-train_fr.tokss \bs
...
(\S\ref{HMM}) word-alignment models.  In \S\ref{IBM4}, we'll see how you can
include a IBM4 word-alignment model and tally its counts along with the other
two methods.

In \S\ref{PI}, we'll see how you can also produce alignment indicator features
...
\begin{alltt}
   ...
   align-words -o sri -ibm 2 ibm2.tm-train.en_given_fr.gz  ibm2.tm-train.fr_given_en.gz \bs
      ./../../corpora/tm-train_fr.lc.gz ./../../corpora/tm-train_en.lc.gz \bs
      2> log.tm-train.ibm2.fr2en.align | gzip > tm-train.ibm2.fr2en.align.gz
   [...]
\end{alltt}
\end{small}
...
\begin{alltt}
   gen-jpt-parallel.sh -n 4 -nw 4 -o jpt.ibm2.tm-train.fr-en.gz -w 1 \bs
      ...
      ../../corpora/tm-train_fr.lc.gz ../../corpora/tm-train_en.lc.gz \bs
      tm-train.ibm2.fr2en.align.gz >& log.jpt.ibm2.tm-train.fr-en
\end{alltt}
\end{small}
...
\begin{alltt}
   ...
   gen-jpt-parallel.sh -n 4 -nw 4 -o jpt.hmm3.tm-train.fr-en.gz -w 1 \bs
      ...
      ../../corpora/tm-train_fr.lc.gz ../../corpora/tm-train_en.lc.gz \bs
      tm-train.hmm3.fr2en.align.gz >& log.jpt.hmm3.tm-train.fr-en
\end{alltt}
\end{small}
...
\begin{alltt}
   ...
   joint2cond_phrase_tables -prune1 100 -v -i -z -hmm -1 fr -2 en \bs
      ...
      -reduce-mem -no-sort jpt.merged.tm-train.fr-en.gz \bs
      >& log.cpt.merged.hmm3-kn3-zn.tm-train.fr2en
\end{alltt}
\end{small}
...
\begin{alltt}
   ...
   joint2multi_cpt -prune1w 100 -v -i -z -1 fr -2 en \bs
      ...
      -ibm_l2_given_l1  hmm3.tm-train.en_given_fr.gz \bs
      -ibm_l1_given_l2  hmm3.tm-train.fr_given_en.gz \bs
      j2m/ibm4 j2m/hmm3 j2m/ibm2 &> log.cpt.PI-kn3-zn.hmm3.fr2en
\end{alltt}
\end{small}
...
\begin{alltt}
   ...
   parallelize.pl -nolocal -psub -1 -n 4 -np 4 -w 100000 \bs
      ...
      'dmcount -ext -v -m 8 -hier \bs
         ../../corpora/tm-train_fr.lc.gz ../../corpora/tm-train_en.lc.gz \bs
         ../tm/tm-train.hmm3.fr2en.align.gz > hldm.hmm3.counts.gz' \bs
      >& log.hldm.hmm3.counts
   parallelize.pl [...] 'dmcount [...] > hldm.ibm2.counts.gz' \bs
      >& log.hldm.ibm2.counts
   parallelize.pl [...] 'dmcount [...] > hldm.ibm4.counts.gz' \bs
      >& log.hldm.ibm4.counts
   merge_multi_column_counts -fillup - \bs
      ...
      | dmestm -s -g hldm.ibm4+hmm3+ibm2.fr2en.bkoff \bs
        -wtu 5 -wtg 5 -wt1 5 -wt2 5 2> log.hldm.ibm4+hmm3+ibm2.fr2en \bs
...
the same key in two files.  The default is to simply concatenate the parts
...
language and translation models forms one whole corpus that is treated together.
This is often not realistic: you may have data that come from different domains,
for example data from web material vs. newswire, or separate specialized
translation memories.  You might also have a large amount of generic data that
we want to combine with smaller, in-domain data.  There are several ways to
combine these corpora into a system that is adapted to a particular domain
while taking advantage of additional training data.
...
they will give a reasonably high score to translation hypotheses that at least
one of their components likes.  In contrast, you could use you different
corpora to create multiple LMs and TMs that are combined in the log-linear
model: in that case, the models all have to agree that a hypothesis is good
before it gets a good score.  The linear combination allows the models to be
used in a kind of fall-back way: if the in-domain model knows the text and
...
training files, as well as the dev and test sets, come from the primary domain,
while the other training sets are from the other two domains.

To simulate this scenario, we've added files to
\url{$PORTAGE/test-suite/tutorial-data/mix}.
They don't actually come from different domains here, but we'll pretend they do
for the sake of our tutorial.  You should copy these files into your
\code{corpora} directory, adding them to the files already there:
\begin{small}
\begin{alltt}
...
The first mixture models we will look at are the mixture language models.  To
tell the framework to use them, you should edit \code{Makefile.params} and
modify the \code{MIXLM} variable to list the stems of the three training sets
for LMs: \code{lm-train1 lm-train2 lm-train3}.  For Mixture LMs, it's also
often useful to include an additional language model trained on the union of
all the corpora together.  Here, \code{lm-train_*} is in fact the union of the
three pretend domains, so we'll throw in \code{lm-train} at the end of
\code{MIXLM}:
\begin{small}
\begin{alltt}
...
we need to set that explicitely: \code{TRAIN_TC = lm-train1}.  Here we are
using the in-domain corpus for truecasing.

With the variables defined as discussed, training of the MixLM is automatic,
but we'll break it down again.
\begin{small}
\begin{alltt}
   > \textbf{make corpora}
   ...
   mx-calc-distances.sh -v em components_fr ../../corpora/dev1_fr.lc \bs
      > dev1.distances 2> log.dev1.mixlm
...
lot of work.  It creates the three per-domain source-side language models, as
...
\begin{alltt}
   ...
   mx-mix-models.sh mixlm dev1.weights components_en \bs
      ../../corpora/dev1_en.lc > dev1.mixlm
\end{alltt}
\end{small}

Having tuned the weights on the source-side, the target-side component LMs are
created and the mixture model is written by combining the weights in
\code{dev1.weights} with the target-side components in \code{components_en}:
\begin{small}
\begin{alltt}
...
violates our assumptions: the EM algorithm recognized that \code{lm-train} is
actually the best corpus to model the dev set and gave it the highest weight.
With real data, you should get more reasonable results.
...
mixture weights for the new document or test set: we call this using a dynamic
mixture model because the weights are dynamically adjusted to new text, which
works well as long as the new document is large enough, containing scores to
...
To enable this MixTMs in the framework, edit \code{Makefile.params} and set
\code{MIXTM} to the three per-domain corpora:
\begin{small}
\begin{alltt}
...
but here it is: the word-alignment model from the first corpus is used in a
special way and it is assumed to be in domain.

As for MixLMs, remove the definition of \code{TRAIN_TM}, since the MixTM
replaces the regular TM.
...
\begin{alltt}
   ...
   [commands to create *.align.gz and jpt.* for the dev1 tuning set, for each alignment method]
   ...
   [repeat merge_counts for tm-train2, tm-train3 and dev1]
   ...
   TODO - complete this once the code is fixed.


\end{alltt}
\end{small}

The MixTM training process can be quite slow, because a lot of models and files
are created, in particular for the full matrix of alignment methods and
component corpora.  These are created in the same way as in \S\ref{TM}, so we
left out most of the details here.  As before, \code{merge_counts} is used to
combine the counts from the JPTs and \code{joint2cond_phrase_tables} is used to
produce CPTs with probabilities estimated from each component corpus, separately.

TODO complete this.

\subsubsection{Generic and Pre-trained Models} \label{Generic}

Here we assume that all the models are trained locally, in the current
instance of the framework.  Sometimes, you might have already trained your
model(s) in other instances of the framework, or they might come from
generic models for which you don't have the training corpus.  In that case, you
can add the name of the target-side pre-trained language models in
\code{.binlm.gz} format to \code{MIXLM_PRETRAINED_TGT_LMS} and they will be
included in the mixture model.  The source-side pre-trained language models are
also required, to tune the mixture weights, and should have the same name
except for the language code.
...
\begin{alltt}
   > \textbf{make all}
   mkdir -p foos
   filter_models -z -r -tm-soft-limit cpt.dev1 -ldm < ../../corpora/dev1_fr.rule \bs
      >& log.cpt.dev1.FILT
   tune.py -v -o canoe.ini.cow.FILT -p 4 -c 1 -m 15 -n 100 -a lmira --workdir=foos \bs
      -f canoe.ini.FILT ../../corpora/dev1_fr.rule ../../corpora/dev1_en.lc \bs
      &> log.canoe.ini.cow
...
\begin{alltt}
   ...
   ce_translate.pl -n=4 -train -src=fr -tgt=en -notok -nolc -nl=s \bs
      ...
      ../../corpora/dev3_fr.lc ../../corpora/dev3_en.lc \bs
      >& log.ce_model.cem
\end{alltt}
\end{small}
...
\begin{alltt}
   ...
   canoe-parallel.sh -n 4 canoe -f canoe.ini.cow.test1 -palign \bs
      < ../corpora/test1_fr.rule 2> log.test1.out \bs
...
\begin{alltt}
   ...
   translate.pl -with-ce -n 4 -notok -nl s -tc -encoding UTF-8 -src fr -tgt en \bs
      -f canoe.ini.cow.test1 -model ce_model.cem ../corpora/test1_fr.al \bs
      > test1.ce 2> log.test1.ce
   [repeat for test2]
\end{alltt}
\end{small}
...
\begin{alltt}
   ...
   rat.sh -lb -n 4 trans -v -K 1000 -msrc ../corpora/test1_fr.rule \bs
      -f canoe.ini.cow.test1 models/rescore/rescore-model \bs
      ../corpora/test1_fr.lc >& log.test1.rat \bs
...
\begin{alltt}
   ...
   truecase.pl -text test1.out.oov -bos -encoding UTF-8 \bs
      ...
      -srclm models/tc/lm-train_fr.nc1.binlm.gz > test1.out.tc 2> log.test1.out.tc
   [repeat for test2]
\end{alltt}
\end{small}
