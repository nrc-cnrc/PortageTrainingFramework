...
sequences of several words (such as allcaps).
...
target-side normalized-case\footnote{``Normalized case'' means that
the first character of the sentence is in lowercase unless the word inherently
requires the upper case.  E.g., ``a flat in London.'' and ``London is on the
Thames.'' are in normalized case.} language models, as well as the main
truecasing LM.  The main truecasing LM and the ``casemap'' are trained on the
normalized-case target-side training corpus.  Together, they are used to
determine the case for the output of the decoder.  Then case information from
the source sentence, including the first word, is transfered to the output of
the decoder.
...
\begin{alltt}
   ...
   [use-ftm]
\end{alltt}
\end{small}\footnote{If you set \code{USE\_LDM=1}, you would also see a line
pointing to the LDM file,
\code{[lex-dist-model-file] models/ldm/ldm.hmm3+ibm2.fr2en.gz};
the \code{[distortion-model]} block would include the six LDM features,
\code{back-lex\#m back-lex\#s back-lex\#d fwd-lex\#m fwd-lex\#s fwd-lex\#d};
and the options \code{[dist-phrase-swap]} and \code{[dist-limit-ext]} would be
enabled.}
...
\code{ce_translate.pl} handles both of these steps:
\begin{small}
\begin{alltt}
   > \textbf{cd translate}
...
If you inspect the \code{test.out}, \code{test.ce} and/or code{test.rat} output
files, you will notice that they contain lowercase, tokenized text. Two
postprocessing steps are required to restore normal case and normally spaced
text: truecasing and detokenizing.
...
If you are using the old truecasing module, the same files will be generated by
\code{truecase.pl}, but the options will be different, and you will not get all
the benefits of the new module.
...
\begin{alltt}
   ...
   grep Human *.bleu
   test.out.bleu:Human readable value: 19.02 +/- 3.29
...
\begin{alltt}
   ...
   13M     models/portageLive/models/tm/cpt.merged.hmm3-rf-zn.train.fr2en.tppt
   ...
   5.3M    models/portageLive/models/lm/train_fr-kn-5g.tplm
   4.9M    models/portageLive/models/lm/train_en-kn-5g.tplm
...
\begin{alltt}
   ...
   13M     models/tm/cpt.merged.hmm3-rf-zn.train.fr2en.tppt
   3.4M    models/lm/train_en-kn-5g.binlm.gz
   5.4M    models/lm/train_en-kn-5g.lm.gz
   4.9M    models/lm/train_en-kn-5g.tplm
   3.7M    models/lm/train_fr-kn-5g.binlm.gz
   5.9M    models/lm/train_fr-kn-5g.lm.gz
   5.3M    models/lm/train_fr-kn-5g.tplm
   448K    translate
   8.0K    models/confidence/ce_model.cem
   16M     models/tc
   98M     total
