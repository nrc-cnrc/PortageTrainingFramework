...
TODO: REVIEW THIS SECTION ONCE THE REST HAS BEEN DONE.
...
\item Model training (\S\ref{Training}): includes language model (\S\ref{LM}),
      translation model (\S\ref{TM}), hierarchical lexicalized distortion mode
      (\S\ref{LDM}), truecasing model (\S\ref{TC}), decoder weight optimization
      (\S\ref{COW}), confidence estimation model training (\S\ref{CE}), and
...
distortion model (\S\ref{LDM}), training a rescoring model (\S\ref{RAT}), and
training a confidence estimation (CE) model (\S\ref{CE}).

TODO THE PARAGRAPH ABOVE NEEDS TO MENTION MIXTM AND MIXLM, AND MAYBE IMPROVE
HOW HLDMS ARE MENTIONED.
...
\code{lm-train} is the simplest option.  If you have access to large amounts of
...
section~\ref{LM}) on this permuted corpus, producing \code{lm-train_en.binlm.gz},
...
With the alignment saved, extracting the joint-count phrase table from the
parallel corpus is reasonably quick.  The make command issued above should have
also launched this:
\begin{small}
\begin{alltt}
...
Now that we have joint-count phrase tables from our IBM2 and HMM
...
If you wish to build and use IBM4 translation models, you should
edit \code{Makefile.params} at the root of the framework, and add \code{IBM4} to
the \code{MERGED_CPT_JPT_TYPES} variable, before the \code{HMM3} and \code{IBM2}
values, which you will find there already.
...
produced (or, at least, their BLEU scores), without merely overfitting the dev
set, as the Powell tuning algorithm tends to do when faced with too many
features.
...
an fourth column containing two or three values (one per aligner), each at 1
(meaning that alignment had the phrase pair) at 0.3 (meaning that alignment did
not).  The 0.3 works as a constant arbitrary penalty that gets applied whenever
a given aligner did not produce a given phrase pair.  The penalty value is
arbitrary here in the phrase table, but its weight gets tuned so as to
maximized BLEU scores on your dev set later.
...
that \code{joint2cond_phrase_table} can, plus a number of ways to exploit the
fact that counts come from different sources.  Here we use it with just one
particular setting that has been proved to be effective empirically.
...
\begin{alltt}
   ...
   parallelize.pl -nolocal -psub -1 -n 4 -np 4 -w 100000 \bs
      -s '../../corpora/tm-train_fr.lc.gz' -s '../../corpora/tm-train_en.lc.gz' ../tm/tm-train.hmm3.fr2en.align.gz \bs
      -merge 'merge_multi_column_counts -' \bs
      'dmcount -ext -v -m 8 -hier \bs
       ../../corpora/tm-train_fr.lc.gz ../../corpora/tm-train_en.lc.gz ../tm/tm-train.hmm3.fr2en.align.gz \bs
       > hldm.hmm3.counts.gz'
   ...
   merge_multi_column_counts - hldm.hmm3.counts.gz hldm.ibm2.counts.gz hldm.ibm4.counts.gz \bs
      | dmestm -s -g hldm.hmm3+ibm2+ibm4.fr2en.bkoff \bs
        -wtu 5 -wtg 5 -wt1 5 -wt2 5 2> log.hldm.hmm3+ibm2+ibm4.fr2en \bs
      | gzip > hldm.hmm3+ibm2+ibm4.fr2en.gz
\end{alltt}
\end{small}
...
allocation.  We generally observe a fair gain in translation quality (as
measured by BLEU) when using HLDMs, although you will need to determine,
...
The rest of this document assumes you did not train HLDMs, so if you modified
your \code{Makefile.params} file to set \code{USE_HLDM = 1}, you should comment
out that line before proceeding to the next section.

TODO MAYBE REMOVE THE ABOVE COMMENT AND USE HLDMs IN TUNING?
...
\begin{alltt}
   ...
   cat canoe.ini.template \bs
      | sed -e 's/<SL>/fr/g' \bs
            -e 's/<TL>/en/g' \bs
            -e 's#<CPTS>#models/tm/cpt.merged.hmm3-kn3-zn.train.fr2en.gz#g' \bs
...
\begin{alltt}
   > \textbf{cat canoe.ini}
   [ttable-multi-prob]
      models/tm/cpt.merged.hmm3-kn3-zn.train.fr2en.gz
   [lmodel-file]
...
\begin{alltt}
   ...
   14M     models/portageLive/models/tm/cpt.merged.hmm3-kn3-zn.train.fr2en.tppt
...
Resource summary for $PORTAGE/toy.experiment:
      ...
      tc:TIME-MEM                                              WALL TIME: 48s      CPU TIME: 25s        VSZ: 0.090G   RSS: 0.015G
         log.cpt.merged.hmm3-kn3-zn.train.fr2en:TIME-MEM       WALL TIME: 14s      CPU TIME: 17s        VSZ: 0.036G   RSS: 0.006G
         log.cpt.merged.hmm3-kn3-zn.train.fr2en.tppt:TIME-MEM  WALL TIME: 15s      CPU TIME: 7s         VSZ: 0.049G   RSS: 0.007G
...
\begin{alltt}
   ...
   12M     models/tm/cpt.merged.hmm3-kn3-zn.train.fr2en.gz
   14M     models/tm/cpt.merged.hmm3-kn3-zn.train.fr2en.tppt
