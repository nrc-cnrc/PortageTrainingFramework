...
the upper case.  E.g., ``his flat in is London.'' and ``London is on the Thames.''
...
The first command above generates \code{train_en.tokss.gz}, a variant
of the target-side training corpus with all-caps sentences (which are misleading
for training truecasing models) removed and sentence splitting
...
second command generates \code{train_en.revtokss.gz}, a permutation of
the target-side training corpus needed to train the target-side
...
\code{train_en-kn-3g.binlm.gz}, the main target-size truecasing LM. The final
