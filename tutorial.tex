...
\item Model training (\S\ref{Training}): includes language model (\S\ref{LM}),
      ...
      rescoring model training (\S\ref{RAT}), and confidence estimation model
      training (\S\ref{CE}).
\item Translating and testing (\S\ref{TranslatingTesting}): includes
      decoding (\S\ref{Decoding}), rescoring (\S\ref{RATTrans}), confidence
      estimation(\S\ref{CETrans}), truecasing (\S\ref{Truecasing}) and testing
      (\S\ref{Testing}).
\end{enumerate}
...
\begin{small}
\begin{alltt}
   > \textbf{make help | grep -B1 irstlm}
   ...
   > \textbf{export PATH=$PORTAGE/pkgs/irstlm/bin:$PATH}
   > \textbf{export IRSTLM=$PORTAGE/pkgs/irstlm}
\end{alltt}
\end{small}
...
\S\ref{COW}).} Commands are assumed to be issued from the
...
\begin{alltt}
   ...
   zcat -f ../../corpora/train_en.lc.gz \bs
      | add-start-end.sh \bs
      | gzip -c > train_en.marked.gz
   Creating IRSTLM train_en-kn-5g.ilm.gz
   build-lm.sh -p -t stat.$$ -n 5 -k 4 -s kneser-ney \bs
      -i "gunzip -c train_en.marked.gz" -o train_en-kn-5g.ilm.gz \bs
      >& log.train_en-kn-5g.ilm
   Creating ARPA text format train_en-kn-5g.lm.gz
   compile-lm --text yes train_en-kn-5g.ilm.gz /dev/stdout \bs
        2> log.train_en-kn-5g.lm \bs
        | egrep -v '^Saving in txt format to' \bs
        | lm_sort.pl \bs
...
\url{$PORTAGE/test-suite/tutorial-data/lm}. You can
...
\begin{alltt}
   ...
   make -C models tc
   make -C tc all
   compile_truecase_map ../../corpora/train_en.tc.gz ../../corpora/train_en.lc.gz \bs
...
\url{$PORTAGE/test-suite/tutorial-data/tc}.  You can
...
\begin{alltt}
   > \textbf{cd models/ldm}
   > \textbf{make all}
   parallelize.pl -nolocal -psub -1 -n 30 -np 30 -w 100000 \bs
...
\begin{alltt}
   ...
   cat canoe.ini.template \bs
      | sed -e 's/<SL>/fr/g' \bs
            -e 's/<TL>/en/g' \bs
            -e 's#<CPTS>#models/tm/cpt.hmm3-rf-zn.train.fr2en.gz models/tm/[...]' \bs
            -e 's#<LMS>#models/lm/train_en-kn-5g.binlm.gz#g' \bs
            -e 's/^\bs(\bs[\bs(weight-f\bs|ftm\bs)\bs]\bs)/##mid##\bs1/' \bs
      | configtool -p "args:-dist-phrase-swap -dist-limit-ext \bs
           -lex-dist-model-file models/ldm/ldm.hmm3+ibm2.fr2en.gz -distorti[...]" \bs
      > canoe.ini
...
BLEU score: 0.176354  -d 1:1:1:1:1:1:1 -w 0 -lm 1 -tm 1:1 -ftm 1:1
BLEU score: 0.031585  -d -0.03:0.01:0.02:-0.01:0.00:0.04:-1 -w 0.00 -lm 0.25 -tm 0.04:0.14 -
BLEU score: 0.014561  -d 0.00:-0.33:-1:0.14:-0.20:-0.04:0.05 -w -0.09 -lm 0.47 -tm -0.00:0.0
BLEU score: 0.100003  -d 1:0.80:-0.83:-0.66:-0.32:0.51:-0.39 -w -0.20 -lm 0.83 -tm -0.00:0.1
BLEU score: 0.044412  -d 0.00:1:-0.00:-0.03:-0.79:0.06:-0.00 -w -0.02 -lm 0.84 -tm -0.15:0.4
BLEU score: 0.174832  -d 0.07:1:0.05:0.23:-0.41:0.11:0.03 -w -0.07 -lm 0.66 -tm -0.04:0.22 -
BLEU score: 0.147715  -d 0.30:-0.05:-0.28:0.03:-0.06:-0.14:-0.02 -w -0.21 -lm 1 -tm 0.11:0.4
BLEU score: 0.191488  -d 0.95:-0.09:-0.19:-0.79:-0.06:-0.15:-0.00 -w -0.21 -lm 1 -tm 0.11:0.
BLEU score: 0.195232  -d 0.39:-0.55:0.27:-0.50:0.29:0.60:0.28 -w -0.21 -lm 1 -tm 0.19:0.29 -
BLEU score: 0.184187  -d 0.31:-0.16:-0.81:-0.04:-0.91:0.87:-0.00 -w 0.01 -lm 1 -tm 0.24:0.08
BLEU score: 0.191695  -d 1:-0.08:-0.81:-0.31:0.32:0.53:0.04 -w -0.16 -lm 0.76 -tm 0.15:0.22 
BLEU score: 0.194884  -d 1:0.44:-0.68:-0.07:-0.05:0.53:0.46 -w -0.48 -lm 0.65 -tm 0.16:0.05 
BLEU score: 0.107006  -d 0.28:1:-0.06:0.13:-0.00:-0.00:0.02 -w -0.11 -lm 0.32 -tm 0.03:-0.08
BLEU score: 0.175858  -d 0.26:1:-0.02:-0.03:-0.03:0.02:-0.01 -w 0.04 -lm 0.12 -tm -0.00:0.06
BLEU score: 0.189801  -d 1:0.27:-0.24:-0.44:-0.10:0.08:-0.03 -w 0.03 -lm 0.29 -tm 0.03:0.11 
\end{alltt}
\end{footnotesize}
...
\code{-ftm}.  There are seven weights after \code{-d} because we used the
standard WordDisplacement model (``distortion penalty''), as well as the
six-feature lexicalized distortion model.
...
\begin{alltt}
   ...
   [ttable-multi-prob] 
      models/tm/cpt.merge.hmm3-rf-zn.train.fr2en.gz
   [lex-dist-model-file] 
      models/ldm/ldm.hmm3+ibm2.fr2en.gz
   [lmodel-file] 
      models/lm/train_en-kn-5g.binlm.gz
   [weight-d] 0.3922:-0.5552:0.2718:-0.5061:0.2949:0.6087:0.2886
   [weight-w] -0.214144
   [weight-l] 1
   [weight-t] 0.199763:0.298726
   [weight-f] -0.013952:0.232236
   [ttable-limit] 30
   [ttable-threshold] 0
   [stack] 20000
   [beam-threshold] 0.001
...
\code{canoe} to use during search. This step sometimes gives a modest
improvement over the results obtained using \code{canoe} alone. Often,
...
\begin{alltt}
   ...
   ok
   Generating test.out
   canoe-parallel.sh -lb -n 4 canoe -f canoe.ini.cow.test \bs
...
\begin{alltt}
   ...
   Generating test.ce
   ce_translate.pl -n=4 -notok -nolc -nl=s -src=fr -tgt=en \bs
      -test=../corpora/test_en.lc -out=test.ce canoe.ini.cow.test \bs
