...
which maps each lower case words to its possible truecase variants, and a
standard language model trained on the truecase corpus.

With version 1.4.3, we have introduced a new variant of truecasing which
carries casing information from the source sentence into the target sentence,
including the casing for the first word in the sentence, and unusual casing for
...
\texttt{TC\_USE\_SRC\_MODELS=1} in the advanced configuration section of
\texttt{Makefile.params}.}  It requires three language models: source-side and
target-side normalized-case\footnote{``Normalized case'' means that the first
character of the sentence is in lowercase unless the word inherently requires
the upper case.  E.g., ``a flat in London.'' and ``London is on the Thames.''
are in normalized case.} language models, as well as the main truecasing LM. 
The target-side normalized-case LM is used only for generating a case-normalized
target-side training corpus; it is not used in translation. The main truecasing
LM and the ``casemap'' are trained on the normalized-case target-side training
corpus.  Together, they are used to determine the case for the output of the
decoder.  Then additional case information from the source sentence, including
the first word, is transfered to that truecased output of the decoder.
...
\begin{alltt}
   ...
   [commands to train normalized-case target-language LM train_en.nc1.binlm.gz]
   ...
   [commands to train normalized-case source-language LM train_fr.nc1.binlm.gz]
\end{alltt}
\end{small}

The first truecasing command above generates \code{train_en.tokss.gz}, a variant
of the target-side training corpus with all caps sentences (which are misleading
for training truecase models) removed and sentence splitting
re-done.\footnote{Sentence splitting is re-applied because corpus text from some
sources such as TMX files may contain lines with multiple sentences, and
beginning-of-sentence detection is important for case normalization.}  The
second command generates \code{train_en.revtokss.gz}, a further permutation of
the target-side training corpus needed for training the target-side
normalized-case LM. The third command invokes \code{make} to train a language
model as in section~\ref{LM}, this time producing \code{train_en.binlm.gz}, the
target-side normalized-case language model. The next command, \code{normc1},
uses a normalized-case LM to case normalize text producing, in this case,
\code{train_en.nc1.gz}, the case-normalized target-side training corpus. Next,
the casemap for the main target-side truecasing model, \code{train_en.map}, is
generated using \code{compile_truecase_map}, which compiles the casemap by
reading the normalized-case and lowercased versions of the corpus
simultaneously. Then, \code{make} is invoked again, this time to produce
\code{train_en-kn-3g.binlm.gz}, the main target-size truecasing model. The final
three commands listed above do the same thing as the first three, but for the
source side instead to produce \code{train_fr.nc1.binlm.gz}, the source-side
normalized-case language model needed by the new truecasing workflow.
...
truecasing module, but we have to disable it temporarily since it is not yet
...
by \code{truecase.pl}, but the options will be different (\code{-src},
\code{-pal}, \code{-srclang}, \code{-srclm} options will be omitted), and you
